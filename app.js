...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    instruction.innerHTML = "<br>Get three or more items in a row and level up once you pass the 'Score To Beat.' Some levels come with new vending machine goodies! Move vending machine items left, right, up, down or diagonally by one step. Start playing now!"
    ...
    let toAdd = 3;
        ...
        function assignSrc(base, srcChng, ol){
            for(let x = 0; x < ol.length; x++){
                if(base === x){
                    srcChng.src = ol[x];
                    console.log("changer",base, srcChng.src);
                }
            }
        }
        ...
        function createBoard(){
            ...
            for(let i =0; i< w*w; i++){
                ...
                // }else{
                //     square.style.width = String(Math.floor((300/w)))+"px";
                //     square.style.height = String(Math.floor((300/w)))+"px";
                //     image.style.width = String(Math.floor((300/w) - 5))+"px";
                //     image.style.height = String(Math.floor((300/w) - 5))+"px";
                // }
        ...
        function dragStart(){
            ...
            console.log("typedragged", typeDragged);
        ...
        function dragEnd(){
            ...
            let validMoves = [Number(squareIdBeingDragged)-1, Number(squareIdBeingDragged)-w, Number(squareIdBeingDragged)+1, Number(squareIdBeingDragged)+w];
                console.log(validMoves, squareIdBeingReplaced);
            ...
            if(validMove && squareIdBeingReplaced ){
                ...
                console.log('invalidMove', typeDragged);
        ...
        function dragDrop(e){
            ...
            console.log("dragdrop", sq[Number(squareIdBeingReplaced)].children[0].src);
        }
        function moveDown(){
            ...
            for(let co = w; co >= 3; co--){
                for (let j = 0; j < sq.length-((co-1)*w);  j++){
                    ...
                    if(colOfN.every(index => sq[index].id.split(':')[1] === selectedType && sq[index].id.split(':')[1] !== isBlank)){
                        ...
                        console.log('checkN', score, colOfN);
                        colOfN.forEach(idx => {
                            sq[idx].children[0].src = '';
                            sq[idx].setAttribute('id', (sq[idx].id.split(':')[0])+":");
            ...
            for(let da = w; da >= 3; da--){
                for (let j = 0; j <= (sq.length-1-((w+1)*(da-1))); j++){
                    ...
                    if(dglOfN.every(index => sq[index].id.split(':')[1] === selectedType && sq[index].id.split(':')[1] !== '')){
                        ...
                        console.log('checkN', score, dglOfN);
                        dglOfN.forEach(idx => {
                            sq[idx].children[0].src = '';
                            sq[idx].setAttribute('id', (sq[idx].id.split(':')[0])+":");
                        });
            ...
            for(let da = w; da >= 3; da--){
                for (let j = 0; j <= (w-1)+(w)*(w-da);  j++){
                    ...
                    if(darOfN.every(index => sq[index].id.split(':')[1] === selectedType && sq[index].id.split(':')[1] !== isBlank)){
                        ...
                        console.log('checkN', score, darOfN);
                        darOfN.forEach(idx => {
                            sq[idx].children[0].src = '';
                            sq[idx].setAttribute('id', (sq[idx].id.split(':')[0])+":");
